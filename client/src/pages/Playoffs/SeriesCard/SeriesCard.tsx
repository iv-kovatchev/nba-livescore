  ...
  return (
    ...
    >
      <div className="series-card__summary" onClick={onToggle}>
        ...
        <div className={`series-card__team ${winner === "team2" ? "series-card__team--eliminated" : ""}`}>
          <img
            className="series-card__logo"
            src={team1.logo ?? `https://a.espncdn.com/i/teamlogos/nba/500/${team1.abbreviation.toLowerCase()}.png`}
            alt={team1.name}
          />
          ...
          <span className={`series-card__wins ${winner === "team1" ? "series-card__wins--champion" : ""}`}>
            {team1Wins}
          </span>
        </div>
        ...
        <div className={`series-card__team ${winner === "team1" ? "series-card__team--eliminated" : ""}`}>
          <img
            className="series-card__logo"
            src={team2.logo ?? `https://a.espncdn.com/i/teamlogos/nba/500/${team2.abbreviation.toLowerCase()}.png`}
            alt={team2.name}
          />
          <span className="series-card__team-name">
          ...
          <span className={`series-card__wins ${winner === "team2" ? "series-card__wins--champion" : ""}`}>
            {team2Wins}
          </span>
        </div>
        ...
        <div className="series-card__footer">
          <div className="series-card__footer-left">
            ...
            <span className="series-card__games-played">
              {gamesPlayed} {t('playoffs.gamesPlayed')}
            </span>
          </div>
          <div className={`series-card__chevron ${expanded ? "series-card__chevron--expanded" : ""}`}>
            <FiChevronDown size={16} />
          </div>
        </div>
      </div>

      <div className={`series-card__games ${expanded ? "series-card__games--expanded" : ""}`}>
        <div className="series-card__games-inner">
          <table className="series-card__table">
            <thead>
              <tr>
                <th>{t('playoffs.game')}</th>
                <th>{team1.city} {team1.name}</th>
                <th>{team2.city} {team2.name}</th>
                <th>{t('playoffs.status')}</th>
              </tr>
            </thead>
            <tbody>
              {series.games.map((game, index) => {
                ...
                return (
                  ...
                  >
                    <td><div className="series-card__game-num">G{index + 1}</div></td>
                    <td>
                      <div className={`series-card__score ${team1Won ? "series-card__score--winner" : ""}`}>
                        {isPlayable ? team1Score : "–"}
                      </div>
                    </td>
                    <td>
                      <div className={`series-card__score ${team2Won ? "series-card__score--winner" : ""}`}>
                        {isPlayable ? team2Score : "–"}
                      </div>
                    </td>
                    <td>
                      <div className={`series-card__game-status series-card__game-status--${game.status}`}>
                        {game.status === "live"
                          ? t('playoffs.live')
                          : game.status === "final"
                            ? t('playoffs.final')
